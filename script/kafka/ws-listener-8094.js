...
function getMessageFromBrodcaster(){
    /* Creaing the web socket server */
    const messageListner = new WebSocket("ws://localhost:8096");
    messageListner.onerror = (error) =>{
        console.log(`Websocket error: ${error}`);
    }

    var strConnectionMessage = '{"headers":{"Authorization":"Bearer TOKEN","oAuthToken": "daf0-VP31XxMn_0","wsSessionID": "vipinXYEBEUH","operation": "offer","source": "expria","Upgrade": "websocket"}, "messageBody":{"id":"123455"}}';
    console.log(strConnectionMessage);

    messageListner.onopen = (error) =>{
        messageListner.send(JSON.stringify(
            {
                "headers":{
                        "Authorization":"Bearer TOKEN",
                        "oAuthToken": "daf0-VP31XxMn_8093",
                        "wsSessionID": "",
                        "operation": "offer",
                        "source": "expria",
                        "Upgrade": "websocket"
    ...
    messageListner.onmessage = (e) =>{
        var strData = JSON.parse(e.data);
        console.log(strData);
    }
}
...
console.log("Node Web-socket Server is running in 8093");